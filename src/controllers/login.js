...
exports.postLogin = async (req, res) => {
    ...
    if (!user) {
        return res.render('user/login', { layout: false, message: "Email không tồn tại!" });
    }

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
        return res.render('user/login', { layout: false, message: "Sai mật khẩu!" });
    }

    if (user.role !== loginType) {
        return res.render('user/login', { layout: false, message: `Tài khoản này không phải ${loginType}!` });
    }
